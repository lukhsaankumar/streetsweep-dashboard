...
export function TicketDetailDrawer({ userName, userId, ticket, onClose, onClaim, onUnclaim, onComplete, allUsers }: TicketDetailDrawerProps) {
  ...
  // Fetch the name of the user who claimed the ticket
  useEffect(() => {
    if (ticket.claimedBy && ticket.claimedBy !== userId) {
      // It's claimed by someone else, fetch their name
      getUserById(ticket.claimedBy)
        .then(user => setClaimedByName(user.name))
        .catch(() => setClaimedByName(null));
    } else {
      setClaimedByName(null);
    }
  }, [ticket.claimedBy, userId]);

  // Get the display name for who claimed/completed the ticket
  const getClaimedByDisplayName = () => {
    if (isClaimedByMe) return userName;
    if (claimedByName) return claimedByName;
    // Fallback to a truncated ID if name lookup failed
    if (ticket.claimedBy) return `User ${ticket.claimedBy.slice(-6)}`;
    return 'Unknown';
  };
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-foreground">
                      {ticket.squad ? ticket.squad.name : getClaimedByDisplayName()}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {ticket.state === 'COMPLETED' ? 'Completed' : 'Claimed'}{' '}
                      {ticket.completedAt || ticket.claimedAt ? formatDate(ticket.completedAt || ticket.claimedAt!) : ''}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 border-t border-border bg-card">
            ...
            {ticket.state === 'CLAIMED' && !isClaimedByMe && (
              <div className="text-center text-muted-foreground py-2">
                This ticket is claimed by {getClaimedByDisplayName()}
              </div>
            )}
